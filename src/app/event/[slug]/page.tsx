      ...
      <section className="relative h-[50vh] md:h-[60vh] overflow-hidden">
        ...
        >
          <div className="container-site w-full">
            ...
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-tight">{event.title}</h1>
          </div>
        </div>
      </section>
      ...
      <div className="container-site py-8 md:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
          <div className="lg:col-span-2 space-y-10">
            ...
            {event.description && (
              <div className="text-zinc-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: formatContent(event.description) }} />
            )}
              ...
              <section>
                ...
                <div className="text-zinc-700 leading-relaxed space-y-1">
                  {event.tipsForVisitors.map((tip, i) => (
                    <p key={i} className="flex items-start gap-2"><span className="text-primary mt-1 shrink-0">•</span>{tip}</p>
                  ))}
                </div>
              </section>
            )}
            {event.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {event.tags.map((tag) => (
                  <Link key={tag} href={`/event/tag/${tag.toLowerCase().replace(/\s+/g, "-")}`} className="text-sm text-zinc-600 bg-zinc-100 hover:bg-zinc-200 px-3 py-1 rounded-full transition-colors">#{tag}</Link>
                ))}
              </div>
            )}
          </div>
            ...
            <div className="lg:sticky lg:top-24 space-y-5">
              {event.images.length > 0 && (
                <section className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
                  <div className="p-4 pb-0"><h3 className="text-sm font-bold text-zinc-900 uppercase tracking-wider">Event Gallery</h3></div>
                  ...
                  <div className="space-y-0">
                    ...
                    {event.officialSite && (
                      <div className="flex items-center justify-between py-3 border-b border-zinc-100 last:border-b-0">
                        ...
                        <a href={event.officialSite} target="_blank" rel="noopener noreferrer" className="text-sm text-primary font-medium hover:underline text-right">Visit website &rarr;</a>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {event.nearestStation && (
                <div className="rounded-xl overflow-hidden border border-zinc-200 aspect-[4/3]">
                  <iframe
                    ...
                    referrerPolicy="no-referrer-when-downgrade" title={`Map showing ${event.nearestStation}`}
                  />
                </div>
              )}
            </div>
