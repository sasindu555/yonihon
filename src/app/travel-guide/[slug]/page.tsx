...
export default async function GuideDetailPage({ params }: Props) {
  const { slug } = await params;
  const [guides, guideCategories] = await Promise.all([getGuides(), getGuideCategories()]);
  const guide = guides.find((g) => g.slug === slug);
  if (!guide) notFound();
    ...
    <>
      ...
      <section className="bg-white border-b border-zinc-100">
        <div className="container-site py-8 md:py-12">
          <Link href={`/travel-guide/category/${guide.categorySlug}`}
          ...
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-zinc-900 leading-tight max-w-3xl">{guide.title}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
          <article className="lg:col-span-2">
            <div className="rounded-xl overflow-hidden mb-8">
              <Image src={guide.image} alt={guide.title} width={1200} height={675} className="w-full h-auto object-cover" />
            </div>
            ...
            {toc.length > 0 && (
              <aside className="bg-zinc-50 border border-zinc-200 rounded-xl p-6 mb-8">
                ...
                <ol className="space-y-1.5">
                  {toc.map((entry, i) => (
                    <li key={entry.anchor}>
                      <a href={`#${entry.anchor}`} className="text-sm text-zinc-600 hover:text-primary transition-colors">
                        {i + 1}. {entry.title}
                      </a>
                    </li>
                  ))}
            ...
            <div className="guide-content text-zinc-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: formatContent(guide.content) }} />
            ...
            <footer className="mt-10 pt-6 border-t border-zinc-200 flex items-center gap-4">
              ...
              <button type="button" className="inline-flex items-center gap-1.5 text-sm text-zinc-600 hover:text-primary transition-colors font-medium">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
          ...
          <aside className="lg:col-span-1">
            <div className="lg:sticky lg:top-24 space-y-6">
              ...
              <div className="bg-white rounded-xl border border-zinc-200 p-5">
                <h3 className="text-sm font-bold text-zinc-900 mb-3 uppercase tracking-wider">Categories</h3>
                <ul className="space-y-2">
                  {guideCategories.map((cat) => (
                    <li key={cat.slug}>
                      <Link href={`/travel-guide/category/${cat.slug}`}
              ...
              <div className="bg-primary rounded-xl p-5 text-white">
                <h3 className="text-sm font-bold mb-2 uppercase tracking-wider">Plan Your Trip</h3>
                <p className="text-sm text-white/80 mb-4">Discover hand-picked Japan workshops curated by our local team.</p>
        ...
        <section className="py-12 md:py-16 bg-zinc-50">
          <div className="container-site">
            <div className="flex items-end justify-between mb-8">
              ...
              <Link href="/travel-guide" className="text-sm text-primary font-semibold hover:underline flex items-center gap-1">
                All guides <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>
              </Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedGuides.map((g) => <ArticleCard key={g.id} article={g} />)}
            </div>
          </div>
        </section>
      )}
