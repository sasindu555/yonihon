...
import { getGuides, getGuideCategories, getPopularTags } from "@/lib/db";
...
export default async function TravelGuidePage({ searchParams }: PageProps) {
  ...
  const [guides, guideCategories, popularTags] = await Promise.all([
    getGuides(),
    getGuideCategories(),
    getPopularTags(),
  ]);
  ...
  return (
    <>
      <Hero
        ...
        size="medium"
      >
        <div className="max-w-md mx-auto"><SearchInput placeholder="Search guides..." /></div>
      </Hero>
        ...
        <div className="container-site">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {guideCategories.map((cat) => (
              <Link key={cat.slug} href={`/travel-guide/category/${cat.slug}`}
                ...
                <span className="text-xs text-zinc-500 block mt-0.5">{cat.count} Articles</span>
              </Link>
            ))}
          </div>
          <div className="mt-4 text-center">
            <Link href="/travel-guide" className="text-sm text-primary font-semibold hover:underline">View all categories</Link>
          </div>
        </div>
      </section>
      ...
      {featuredGuides.length > 0 && (
        <section className="py-12 bg-zinc-50">
          <div className="container-site">
            <h2 className="text-2xl font-bold text-zinc-900 mb-6">Featured Articles</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {featuredGuides.map((guide) => <ArticleCard key={guide.id} article={guide} variant="featured" />)}
            </div>
          </div>
        </section>
      )}
        ...
        <div className="container-site">
          ...
          {guides.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {guides.map((guide) => <ArticleCard key={guide.id} article={guide} />)}
            </div>
          ) : (
            <div className="text-center py-12 text-zinc-500"><p>No articles yet. New stories drop every week.</p></div>
          )}
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              ...
              <div className="flex flex-wrap gap-2">
                {popularTags.map((tag) => (
                  <Link key={tag} href={`/travel-guide?tag=${tag.toLowerCase().replace(/\s+/g, "-")}`}
            ...
            <div><div className="lg:sticky lg:top-24"><NewsletterForm /></div></div>
          </div>
        </div>
      </section>
    </>
